...
//   CalcDriftRateOutputArray=calculatedriftrate(1*Tracker.DayNow.value,1*Tracker.MonthNow.value,1*Tracker.YearNow.value,1*Tracker.HourNow.value,
//         1*Tracker.MinuteNow.value,1*Tracker.SecondNow.value,Tracker.Satellite_rev_solar_day.value,Tracker.Satellite_rev_sidereal_day.value,
//         Tracker.first_derative_mean_motion.value,Tracker.Eccentricity.value,Tracker.Epoch.value,Tracker.Meananomaly.value,Tracker.Argument_of_perigee.value,Tracker.RA_AN.value,Tracker.Inclination.value,Tracker.NumberOfOrbitsAtEpoch.value);
...
//              Tracker.LongitudeDriftrate.value=getLonAsEast(CalcDriftRateOutputArray[49],CalcDriftRateOutputArray[48],CalcDriftRateOutputArray_after_24_hours[49],CalcDriftRateOutputArray_after_24_hours[48]);
...
//     Tracker.EW_oscillation_caused_by_eccentricity.value=CalcDriftRateOutputArray[47];
//                 //calculate_geocentric_ECI_coordinates(SiteLon,SiteLat,Re,altitude,GMST)
//     Tracker.EarthSiteGeoCentricPositionArray=calculate_geocentric_ECI_coordinates(EarthSiteLongitude,EarthSiteLatitude,Earth_equatorial_radius,0,CalcDriftRateOutputArray[15]);
...
// getCurrentTimeArray=setCurrentTime(1*Tracker.DayNow.value,1*Tracker.MonthNow.value,1*Tracker.YearNow.value,1*Tracker.HourNow.value,
...
// listing+="\n"+i+chr+time_format+chr+Time_since_ep+chr+S_lon+chr+S_lat+chr+Az+chr+El+chr+Range+chr+GeoAlt+chr+AvrGeoAlt+chr+velocity+chr+Drift_rate+chr+Drift_rate_dir+chr+Avr_driftrate+chr+Avr_driftrate_dir;
...
// Tracker.GMST2.value=Rev(( w+   15* (    ( 24110.54841+8640184.812866*Tu+0.093104*Tu*Tu- ((6.2/(1000000))*Tu*Tu*Tu)) /(60*60) )               ));
...
// average_drift_rate=(((-3/2)*(average_distance_from_geo_sync_radius)*earth_rev_pr_solar_day/GeoSyncRadius)*360);  // her går det galt...
...
// Tracker.Altitude.value=Math.pow(    (6028.9* (1440/satellite_rev_pr_sidereal_day)), (2/3)   )  -Earth_equatorial_radius;
...
// Tracker.Driftrate.value=Rev((Tracker.Satellite_rev_solar_day.value*1+Tracker.Earth_rev_solar_day.value*1)*360);
